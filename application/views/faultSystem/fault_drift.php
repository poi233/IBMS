                        ...
                        <div class="tab-content">
                            <div id="tab-1" class="tab-pane active">
                                <div class="panel-body ">
                                    <form method="post" action="<?= site_url('FaultManage/Fault/driftFaultSend') ?>" id="driftFaultForm" name="driftFaultForm">
                                        <fieldset class="form-horizontal">
                                            ...
                                            <div class="form-group"><label class="col-sm-2 control-label">缺陷级别:</label>
                                                <div class="col-sm-2">
                                                    <select class="form-control" name="faultLevel" id="faultLevel">
                                                        <option value="0" <?php if($fault->fault_level==0): ?>selected="selected"<?php endif; ?>>低</option>
                                                        <option value="1" <?php if($fault->fault_level==1): ?>selected="selected"<?php endif; ?>>中</option>
                                                        <option value="2" <?php if($fault->fault_level==2): ?>selected="selected"<?php endif; ?>>高</option>
                                                    </select>
                                            ...
                                            <div class="form-group"><label class="col-sm-2 control-label">缺陷描述:</label>
                                                <div class="col-sm-9">
                                                    <textarea class="form-control" rows="3" name="faultDetail" id="faultDetail" required="required"><?=$fault->fault_detail?></textarea>
                                                </div>
                                            ...
                                            <div class="form-group"><label class="col-sm-2 control-label">缺陷重现:</label>
                                                <div class="col-sm-9">
                                                    <input type="text" class="form-control" placeholder="" name="faultReappearInfo" id="faultReappearInfo" required="required" value="<?= $fault->fault_reappear_info ?>">
                                                </div>
                                                ...
                                                <div class="col-sm-3">
                                                    <select class="form-control" name="projectID" id="projectID">
                                                        <?php foreach($project->result() as $projectRow): ?>
                                                            <option value="<?= $projectRow->project_id ?>" <?php if($fault->project_id==$projectRow->project_id): ?>selected="selected"<?php endif; ?>><?=  $projectRow->project_name?>(<?= $projectRow->project_id ?>)</option>
                                                        <?php endforeach; ?>
                                                    </select>
                                                    ...
                                                    <select class="form-control" name="checkerID" id="checkerID">
                                                        <?php foreach($user->result() as $userRow): ?>
                                                            <?php if($userRow->user_id!=$this->session->userdata('user_id')): ?>
                                                                <option value="<?= $userRow->user_id ?>" <?php if($fault->checker_id==$userRow->user_id): ?> selected="selected"<?php endif; ?>><?= $this->User_model->get_account_by_id($userRow->user_id) ?>
                                                                </option>
                                                            <?php endif; ?>
                                                        <?php endforeach; ?>
                                                    </select>
